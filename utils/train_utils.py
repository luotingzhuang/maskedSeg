...
def random_mask_patches_3d(
    ...
    patch_indices = np.random.choice(
        range(total_patches), num_patches_to_mask, replace=False
    )
    ...
    for index in patch_indices:
        ...
        mask[
            :,
            z_idx * patch_size[0] + offset_x : (z_idx + 1) * patch_size[0] + offset_x,
            y_idx * patch_size[1] + offset_y : (y_idx + 1) * patch_size[1] + offset_y,
            x_idx * patch_size[2] + offset_z : (x_idx + 1) * patch_size[2] + offset_z,
        ] = 1
