...
def eval(all_cases, idx, model, n=100, thresholds=0.6):
...
if __name__ == "__main__":
    ...
    for exp in tqdm(all_exp):
        ...
        for idx in tqdm(range(len(all_cases))):
            ...
            np.save(os.path.join(result_dir,f'majority_thresh{thresh}', f"{all_cases[idx]}.npy"), majority_seg)
            np.save(os.path.join(result_dir,f'mean_thresh{thresh}' ,f"{all_cases[idx]}.npy"), mean_seg)
